...
async def search_with_brightdata(query, session):
    """使用Bright Data SERP API进行搜索 - POST请求格式"""
    ...
    try:
        async with session.post(BRIGHTDATA_API_URL, headers=HEADERS, json=payload, timeout=60) as resp:
            if resp.status == 200:
                result = await resp.json()
                # Bright Data返回的结果中HTML在 'body' 或 'html' 字段
                html = result.get('body', result.get('html', ''))
                return html
            else:
                error_text = await resp.text()
                print(f"    ❌ API错误: {resp.status} - {error_text[:200]}")
                return None
...
async def main():
    print("=" * 70)
    print("🎯 TalentIntel - Bright Data SERP API 搜索 (修正版)")
    print("=" * 70)
    ...
    if unique_candidates:
        ...
        print("\n⚠️ 本次搜索未找到符合条件的新候选人")
        print("建议: 尝试调整搜索关键词或降低分数阈值到0.70")
